...
await page.evaluate(() => {
  const btn = [...document.querySelectorAll('#options-menu .opt-btn')]
    .find((b) => /controller|手柄|手把|コントローラー|컨트롤러|геймпад/i.test(b.textContent));
  btn?.click();
});
await sleep(500);
...
await page.evaluate(() => { for (let i = 0; i < 20 && window.__game.hud.closeAll(); i++) { /* close all */ } });
await sleep(300);
...
await page.evaluate(() => {
  ...
  const g = window.__game.gamepad, sim = window.__game.sim, input = window.__game.input;
  for (let i = 0; i < 60; i++) {
    g.poll(1 / 20);
...
console.log(`left-stick walk: moved ${moved.toFixed(2)} yd (start ${start.x.toFixed(1)},${start.z.toFixed(1)} -> end ${end.x.toFixed(1)},${end.z.toFixed(1)})`);
await page.screenshot({ path: 'tmp/gamepad_walk.png' });
console.log('wrote tmp/gamepad_walk.png');

await browser.close();
console.log(moved > 1 ? 'PASS: gamepad left stick moved the character' : 'WARN: character did not move');
