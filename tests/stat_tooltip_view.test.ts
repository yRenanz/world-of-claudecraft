...
describe('statEffectText number formatting + key selection', () => {
  it('whole-number kinds print as integers', () => {
    for (const kind of ['attackPower', 'rangedAttackPower', 'armor', 'maxHealth', 'maxMana', 'healthRegen', 'manaRegen'] as const) {
      expect(statEffectText({ kind, value: 24 }, deps)).toBe(`hudChrome.statInfo.effects.${kind}(value=24)`);
    }
  });

  it('chance and dps-contribution kinds keep one decimal', () => {
    for (const kind of ['critPct', 'dodgePct', 'spellCritPct', 'dpsFromAp'] as const) {
      expect(statEffectText({ kind, value: 1.1 }, deps)).toBe(`hudChrome.statInfo.effects.${kind}(value=1.1)`);
    }
  });

  it('damage reduction splices both the reference level (integer) and the percent (one decimal)', () => {
    expect(statEffectText({ kind: 'damageReduction', value: 33.3, level: 20 }, deps))
      .toBe('hudChrome.statInfo.effects.damageReduction(level=20,value=33.3)');
  });
});

describe('statBreakdownHeader', () => {
  it('renders "From your N {stat}" only for a primary stat that has effects', () => {
    expect(statBreakdownHeader(model({ stat: 'agi', isPrimary: true, statValue: 22, effects: [{ kind: 'armor', value: 44 }] }), deps))
  ...
  it('is empty for a derived stat or a primary with no effects', () => {
    expect(statBreakdownHeader(model({ stat: 'armor', isPrimary: false, effects: [{ kind: 'damageReduction', value: 1, level: 1 }] }), deps)).toBe('');
...
describe('statNoteTexts', () => {
  it('emits the active notes in display order', () => {
    expect(statNoteTexts(model({ minorForClass: true, baseChanceNote: true, dpsApproxNote: true }), deps)).toEqual([
...
describe('statTooltipHtml', () => {
  it('renders title, prose body, the breakdown header, gain/neutral effect lines, then notes', () => {
    const html = statTooltipHtml(model({
      stat: 'agi',
      isPrimary: true,
      statValue: 22,
      effects: [
    ...
    expect(html).toContain('<div class="tt-bd-head">hudChrome.statInfo.fromYour(value=22,stat=itemUi.stats.agi)</div>');
    expect(html).toContain('<div class="tt-green">hudChrome.statInfo.effects.attackPower(value=22)</div>');
    expect(html).toContain('<div class="tt-green">hudChrome.statInfo.effects.critPct(value=1.1)</div>');
  });

  it('classifies informational effects (regen / damage reduction / dps) as neutral tt-stat', () => {
    const html = statTooltipHtml(model({
      stat: 'sta',
      effects: [{ kind: 'maxHealth', value: 100 }, { kind: 'healthRegen', value: 20 }],
    }), deps);
    expect(html).toContain('<div class="tt-green">hudChrome.statInfo.effects.maxHealth(value=100)</div>');
    expect(html).toContain('<div class="tt-stat">hudChrome.statInfo.effects.healthRegen(value=20)</div>');
  });

  it('appends note lines as tt-sub and omits the header for a derived cell', () => {
    const html = statTooltipHtml(model({ stat: 'critChance', isPrimary: false, statValue: 5.5, baseChanceNote: true }), deps);
...
describe('statTooltipAria', () => {
  it('is a plain-text join (description, header, effects, notes) with no markup', () => {
    const aria = statTooltipAria(model({
      stat: 'agi',
      isPrimary: true,
      statValue: 22,
      effects: [{ kind: 'attackPower', value: 22 }, { kind: 'armor', value: 44 }],
      baseChanceNote: false,
    }), deps);
...
describe('statCellHtml', () => {
  it('builds a focusable, aria-described cell whose value matches the model and whose hidden text is the aria breakdown', () => {
    ...
    expect(html).toContain('class="stat-cell" data-stat="agi" tabindex="0" aria-describedby="statdesc-agi"');
