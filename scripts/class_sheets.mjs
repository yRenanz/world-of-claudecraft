...
const CLASSES = ['warrior', 'paladin', 'hunter', 'rogue', 'priest', 'mage', 'warlock', 'shaman', 'druid'];
const THEME = {
  warrior: ['#ff9d6e', '#150f0c'], paladin: ['#f6d98a', '#15110b'], hunter: ['#bfe89a', '#0f140d'],
  rogue: ['#7fe0a0', '#0d130f'], priest: ['#9ad0ff', '#0d1018'], mage: ['#8fd0ff', '#0b1018'],
  warlock: ['#c79bff', '#100b18'], shaman: ['#6fd0e0', '#0c1014'], druid: ['#9fe07f', '#0d130c'],
};
const TITLE = { warrior: 'Warrior', paladin: 'Paladin', hunter: 'Hunter', rogue: 'Rogue', priest: 'Priest', mage: 'Mage', warlock: 'Warlock', shaman: 'Shaman', druid: 'Druid' };

const b64 = (p) => fs.readFileSync(p).toString('base64');
const esc = (s) => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
const cleanDesc = (s) => String(s || '').replace(/\$d/g, 'X').replace(/\$N/g, 'you').replace(/\$C/g, 'your class').replace(/\$s\d?/g, 'X');

function iconIds(cls) {
  return fs.readdirSync(path.join(SKILLS_DIR, cls)).filter((f) => f.endsWith('.png')).map((f) => f.replace('.png', ''));
}
...
function buildSheet(cls, abil, barDataUrl) {
  ...
  ids.sort((a, b) => (abil[a]?.learnLevel ?? 99) - (abil[b]?.learnLevel ?? 99) || (abil[a]?.name ?? a).localeCompare(abil[b]?.name ?? b));
  const cards = ids.map((id) => {
    const a = abil[id] || {};
    const name = esc(a.name || id);
    const desc = esc(cleanDesc(a.description));
    const lvl = a.learnLevel ? `Lv ${a.learnLevel}` : '';
    const data = b64(path.join(SKILLS_DIR, cls, `${id}.png`));
    return `<div class="card"><img src="data:image/png;base64,${data}" alt="${name}"/>
  ...
  const bar = barDataUrl ? `<div class="barwrap"><div class="barlabel">In-game action bar — ${TITLE[cls]}, level 20</div><img class="bar" src="${barDataUrl}"/></div>` : '';
...
const abil = await page.evaluate(async () => {
  ...
  const out = {};
  for (const [id, a] of Object.entries(m.ABILITIES)) out[id] = { name: a.name, description: a.description ?? '', learnLevel: a.learnLevel ?? 1 };
  return out;
});
...
for (const cls of CLASSES) {
  ...
  try {
    ...
    console.log(cls, `- wrote sheet (${(dims/1024).toFixed(0)}KB)${barDataUrl ? ' + bar' : ' (no bar)'}`);
  } catch (e) {
