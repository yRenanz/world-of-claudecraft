...
const scene = await page.evaluate(() => {
  ...
  me.maxHp = 999999; me.hp = 999999;
  sim.setPlayerLevel(7);
  const p = sim.groundPos(grix.pos.x, grix.pos.z - 4.5);
  me.pos = p; me.prevPos = { ...p };
  me.facing = 0; me.prevFacing = 0;
  sim.targetEntity(grix.id);
  return { grix: true, name: grix.name, level: grix.level, rare: !!grix.rare, elite: !!grix.elite, hp: grix.maxHp };
});
...
await page.evaluate(() => { window.__game.input.camDist = 8; window.__game.input.camPitch = 0.28; });
await sleep(1200);
...
await page.evaluate(() => {
  ...
  for (const id of ['heroic_strike', 'thunder_clap', 'cleave']) { try { g.sim.castAbility(id); } catch {} }
});
await sleep(1600);
